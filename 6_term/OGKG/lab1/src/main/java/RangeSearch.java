...
public class RangeSearch<Key extends Comparable<Key>, Value>  {
    ...
    public class Node {
        public Key y;            // key
        public Value x;          // associated data
    ...
    private void range(Node x, Interval<Key> interval, Queue<Key> list) {
        if (x == null) return;
        if (!less(x.y, interval.min()))  range(x.left, interval, list);
        if (interval.contains(x.y))      list.remove(x.y);
        if (!less(interval.max(), x.y))  range(x.right, interval, list);
    }
    ...
    private boolean isBST(Node x, Key min, Key max) {
        if (x == null) return true;
        if (less(x.y, min) || less(max, x.y)) return false;
        return isBST(x.left, min, x.y) && isBST(x.right, x.y, max);
    }
