...
public class CheckerRunnable implements Runnable {
    ...
    private final int[] aCount = new int[Program.getStringBuilders().length];
    private final int[] bCount = new int[Program.getStringBuilders().length];

    public void run() {
        Arrays.fill(aCount, 0);
        Arrays.fill(bCount, 0);
        for (int i = 0; i < Program.getStringBuilders().length; i++) {
            countAB(Program.getStringBuilders()[i], i);
        }
        if (checkIfDone()) {
            for (int i = 0; i < Program.getThreads().length; i++) {
                Program.getThreads()[i].interrupt();
                System.out.println(Program.getStringBuilders()[i].toString());
            }
        }
    }
