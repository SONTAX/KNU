...
public class PageFault {
    ...
    public static void replacePage(Vector<Page> mem, int replacePageNum, ControlPanel controlPanel) {
        ...
        for (int i = 0; i < mem.size(); i++) {
            if (mem.elementAt(i).physical == -1) {
                physicalPageNum++;
            }
        }
        if (physicalPageNum < (mem.size() / 2 + 1)) {
            int oldestUsedPage = -1;
            int oldestUsedTime = 0;
            int firstPage = -1;
            for (Page page : mem) {
                if (page.physical != -1) {
                    if (firstPage == -1) {
                        firstPage = mem.indexOf(page);
                    }
                    if (page.lastTouchTime > oldestUsedTime) {
                        oldestUsedTime = page.lastTouchTime;
                        oldestUsedPage = mem.indexOf(page);
                    }
                }
            }
            if (oldestUsedPage == -1) {
            ...
            Page page = mem.elementAt(oldestUsedPage);
            ...
            Page page = mem.elementAt(replacePageNum);
            page.physical = mem.size() - physicalPageNum;
